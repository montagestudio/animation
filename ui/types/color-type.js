...
var ColorType = exports.ColorType = Montage.specialize(null, {
    ...
    interpolate: {
        value: function (from, to, f) {
            var alpha = AnimationMath.clamp(Interpolation.interp(from[3], to[3], f), 0, 1);
    ...
    _hsl2rgb: {
        value: function (h, s, l) {
            ...
            m1 = l * 2 - m2;
            return [
                Math.ceil(ColorType._hue2rgb(m1, m2, h + 1 / 3) * 255),
                Math.ceil(ColorType._hue2rgb(m1, m2, h) * 255),
                Math.ceil(ColorType._hue2rgb(m1, m2, h - 1 / 3) * 255)
            ];
        }
    },
        ...
        value: function (value) {
            ...
            if (regexResult) {
                ...
                for (i = 0; i < 3; i++) {
                    ...
                    if (type === "rgb") {
                        out[i] = AnimationMath.clamp(Math.round(parseInt(regexResult[i], 10) * m), 0, 255);
                    } else {
                        out[i] = parseInt(regexResult[i], 10);
                    }
                }
                if (type === "hsl") {
                    out = this._hsl2rgb.apply(null, out);
                }
